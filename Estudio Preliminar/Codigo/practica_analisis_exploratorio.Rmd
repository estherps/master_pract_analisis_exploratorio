...
barplot(studytimeF, main= "Studytime", xlab= "hours" , ylab="%", names.arg= c("< 2", "2 to 5", "5 to 10", ">10"), cex.names=1)
```
...
Realizamos un análisis cluster con el método jerárquico aglomerativo con la forma de agrupación "average".
```{r }
sele <- studentMat[c(2, 14, 15)]
#hc <- hclust(dist(c(33, 2,3, 5, 7, 9, 14, 15, 16, 17, 23, 25, 26, 30)), "ave")
hf <- hclust(dist(sele), "ave")
plot(hf)

```
En el dendograma podemos observar 3 grupos diferenciados formados por:
1- Variables relacionadas con el colegio y estudio.
2- Variables con información de los padres.
...
También observamos que la variable de consumir alchohol durante el fin de semana está directamente correlacionada con la variable salir con los amigos. Es decir, que cuanto más se sale con amigos, más alcohol se consume el fin de semana.
Nuestra variable objetivo G3, que es la nota final, está inversamente correlacionado con Edad y Salir con los amigos. Es decir, alumnos de menor edad sacarán mejores notas y además cuanto más se salga con amigos menos nota final se obtendrá.

Las variables que influyen en la nota final son las variables del ultimo modelo (stepmodel), las siguientes variables: Sex, Medu, studytime, failures, romantic:yes, goout y absences explican mejor el modelo pero siempre contando con el resto de variables del modelo.
